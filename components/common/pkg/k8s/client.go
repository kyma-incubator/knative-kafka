...
func getK8SConfig(logger *zap.Logger) *rest.Config {
	...
	if err == nil && k8sConfig != nil {
		logger.Info("Successfully Acquired Kubernetes In-Cluster Config")
		return k8sConfig
	} else {
	...
	kubeconfig := getKubeConfig(logger)
...
func getKubeConfig(logger *zap.Logger) string {
	...
	if len(kubeconfig) == 0 {
		...
		if err != nil || currentUser == nil {
			logger.Error("Environment Variable 'KUBECONFIG' Not Set - Failed To Get Current User", zap.Error(err))
		} else {
			logger.Info("Environment Variable 'KUBECONFIG' Not Set - Defaulting To ~/.kube/config")
