...
import (
	...
	versioned "github.com/kyma-incubator/knative-kafka/components/controller/pkg/client/clientset/versioned"
	knativekafkav1alpha1 "github.com/kyma-incubator/knative-kafka/components/controller/pkg/client/listers/knativekafka/v1alpha1"
	zap "go.uber.org/zap"
...
type reconcilerImpl struct {
	...
	Lister knativekafkav1alpha1.KafkaChannelLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister knativekafkav1alpha1.KafkaChannelLister, recorder record.EventRecorder, r Interface) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.KafkaChannel, desired *v1alpha1.KafkaChannel) error {
	...
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		...
		if attempts > 0 {
			existing, err = r.Client.KnativekafkaV1alpha1().KafkaChannels(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.KnativekafkaV1alpha1().KafkaChannels(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.KafkaChannel) error {
	...
	_, err = r.Client.KnativekafkaV1alpha1().KafkaChannels(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.KafkaChannel) error {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.KafkaChannel, reconcileEvent reconciler.Event) error {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
