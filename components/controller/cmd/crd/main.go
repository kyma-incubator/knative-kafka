...
func main() {
	...
	// Add Knative Eventing Custom Types To The Manager's Scheme
	err = eventingv1alpha1.AddToScheme(mgr.GetScheme())
	if err != nil {
		logger.Fatal("Failed To Add Knative Eventing Custom Types To Manager's Scheme", zap.Error(err))
	}
	...
	// Add Kafka Channel/Subscription Controllers To Manager
	logger.Info("Adding Controllers To Manager")
	if err := controller.AddToManager(mgr, adminClient); err != nil {
	...
	// Give The Channel & Subscription Controllers A Chance To Shutdown
	kafkachannel.Shutdown(logger)
	kafkasubscription.Shutdown(logger)
